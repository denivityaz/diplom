...
const CourseEdit = ({project}) => {
  ...
  const handleAdd = async () => {
    if(!project)
    try {
      const response = await axios.post('http://185.218.0.16:3333/api/course/', courseData, {
        headers: {
    ...
    try {
      const response = await axios.put(`http://185.218.0.16:3333/api/course/${project.id}`, courseData, {
        headers: {
          'Content-Type': 'application/json',
          authorization: Cookies.get('token')
        }
